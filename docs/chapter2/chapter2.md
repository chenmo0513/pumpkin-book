...
训练集：一、监督学习 supervised learning，

预测值： 1、classfication,离散值；2、regression，回归，连续值。

根据输出数据又分为回归问题（regression）和分类问题（classfication），回归问题的输出通常是连续的数值，分类问题的输出通常是几个特定的数值。

二、无监督学习解决的主要是“聚类”和“降维”问题。


聚类" (clustering) ，即将训练集中的西瓜分成若干组，每组称为 个"簇" (cluster）。典型算法：K均值聚类
